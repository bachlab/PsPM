function sts = scr_exp(modelfile, target, statstype, delim)
...
%
% FORMAT:
% scr_exp(modelfile, target, statstype)
%
% mandatory argument
...
% $Id: scr_exp.m 701 2015-01-22 14:36:13Z tmoser $
% $Rev: 701 $
...
for iFile = 1:numel(modelfile)
    [sts, data(iFile), modeltype{iFile}] = scr_load1(modelfile{iFile}, statstype);
    if sts == -1, return; end;
    if iFile > 1
