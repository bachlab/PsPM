function [ sts, out ] = pspm_convert_visangle2sps(fn, options)
  ...
  %         ┃         Acceptable values
  %         ┣━ 'add'  [default] add the channel at the end
  %         ┗━ 'replace' replace the corresponding channel
  %
  % OUTPUTS
  ...
  %% check missing input
  if nargin < 1
    warning('ID:invalid_input', 'Nothing to do.');
    return;
  elseif nargin < 2
    channels = 0;
  end
