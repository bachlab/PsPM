...
% ---------------------------------
%
% raw.PupilDiameter_Min:                 Minimum allowable pupil size. Pupil values
%                                        less than this value will be marked as
%                                        invalid.
%                                        (Default: 1.5)
%
% raw.PupilDiameter_Max:                 Maximum allowable pupil size. Pupil values
%                                        greater than thin value will be markes as
%                                        invalid.
%                                        (Default: 9.0)
%
...
%                                        temporal width of the island is less than this
%                                        value, all the samples in the island will be
%                                        marked as invalid.
%                                        (Default: 50 ms)
%
...
% --------------------------------
%
% raw.dilationSpeedFilter_MadMultiplier: Number of medians to use as the cutoff
%                                        threshold when applying the speed filter.
%                                        (Default: 16)
%
% raw.dilationSpeedFilter_maxGap_ms:     Only calculate the speed when the gap between
...
%                                        (Default: 50 ms)
%
% raw.gapPadding_forward:                The section right after the end of a gap
%                                        within which samples are to be rejected.
%                                        (Default: 50 ms)
%
...
% These samples are the input for this filter. The dilation speed filter
% will not reject samples that do not feature outlying speeds, such as
% is the case when these samples are clustered together. As such, a
% deviation from a smooth trendline filter is warranted.
%
...
%                                        times the median.  After each pass, all the
%                                        input samples that are outside the threshold
%                                        are removed. Note that all samples (even the
%                                        ones which may have been rejected by the
...
%
% raw.residualsFilter_interpFs:          Fs for first order Butterworth filter.
%                                        (Default: 100 Hz)
%
% raw.residualsFilter_lowpassCF:         Cutoff frequency for first order Butterworth
%                                        filter.
%                                        (Default: 16 Hz)
%
% (Keep filter data)
% ------------------
%
% raw.keepFilterData:                    If true, intermediate filter data will be
%                                        stored.  Set to false to save memory and
...
% ----------------------
%
% valid.interp_upsamplingFreq:           The upsampling frequency used to generate the
%                                        smooth signal.
%                                        (Default: 1000 Hz)
%
...
%                                        interpolated over distances larger than this
