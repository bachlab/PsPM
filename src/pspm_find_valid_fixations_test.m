...
=======
      ...
      for i=1:length(eyes)
        ...
        data{3 + n_chans*(i-1)}.data = pupil;
        data{3 + n_chans*(i-1)}.header.chantype = ['pupil_pp_', e];
        data{3 + n_chans*(i-1)}.header.units = 'diameter';
        data{3 + n_chans*(i-1)}.header.sr = sr;
...
>>>>>>> Stashed changes
    ...
    function test_gaze_validation(this, distance, ...
        ...
        for i=1:numel(degs)
            ...
            for j=1:length(eyes)
                ...
                missing_chan = find(cellfun(@(x) strcmpi(x.header.chantype, ['pupil_missing_', e]), data), 1, 'last');
                pupil_chan = find(cellfun(@(x) strcmpi(x.header.chantype, ['pupil_', e]), data), 1, 'last');
    ...
    function invalid_input(this)
        ...
        % check bitmap option
        bitmap = 'Hello World!';
        this.verifyWarning(@() pspm_find_valid_fixations(fn, bitmap,options), 'ID:invalid_input');
