...
% Base physioevent pattern (JSON/TSV extension added later)
if isempty(task_name)
    base_pattern = sprintf('%s_ses-%s_*physioevents', subject_id, session_id);
else
    base_pattern = sprintf('%s_ses-%s_task-%s_physioevents', subject_id, session_id, task_name);
end

% Add JSON/TSV extensions
json_files = dir(fullfile(physio_eye_path, base_pattern + ".json"));
tsv_files  = dir(fullfile(physio_eye_path, base_pattern + ".tsv"));

% Check existence
if isempty(json_files) || isempty(tsv_files)
    if isempty(task_name)
        warning('No blink/saccade events in %s. Skipping event processing.', physio_eye_path);
    else
        warning('No blink/saccade events for task "%s" in %s. Skipping event processing.', ...
            task_name, physio_eye_path);
    end
else
   % Construct final full file paths
    events_json_filepath = fullfile(physio_eye_path, json_files(1).name);
    events_tsv_filepath  = fullfile(physio_eye_path, tsv_files(1).name);
    
    % Checks if the event files exist
    if ~isfile(events_json_filepath) || ~isfile(events_tsv_filepath)
        warning('Event file %s does not exist.', events_json_filepath);
    else
