...
physio_signals = { 'ecg','eda', 'event'}; % add later -> 'ppg', no more eda = scr && and events are | what is with 'resp' ???
...
for i = 1:num_signals - 1  % Exclude 'events' for now
    ...
    % Construct filenames
    physio_json_filename = sprintf('%s_ses-%s_task-%s_recording-%s_physio.json', subject_id, session_id, task_name, signal);
    physio_tsv_filename = sprintf('%s_ses-%s_task-%s_recording-%s_physio.tsv', subject_id, session_id, task_name, signal);
    ...
    tmp = ''; % find something more elegant 
    if strcmp(signal,'eda') ; tmp = signal ; signal = 'scr'; end % find something more elegant 
    if isfield(physio_json, signal) && isfield(physio_json.(signal), 'Units') % what if lowercase? eda problem!! eda -> column is scr
    ...
    if strcmp(tmp , 'eda'); signal = tmp ; end % find something more elegant
...
duration = events_table.onset(end) - events_table.onset(1); %  WRONG???
