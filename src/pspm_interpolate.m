function [sts, outdata] = pspm_interpolate(indata, options)
	...
	%                       (Default is 0)
	%       .trim_nan:      An option used to determine whether to trim the
	%                       out-of-range values. If set to 1, the out-of-range 
	%                       values at the beginning or the end will be trimed 
	%                       for interpolation, but the original data will not 
	%                       be affected.
	%                       (Default is 0)
	% OUTPUT:
	...
	% set options ---
	try options.overwrite
	catch, options.overwrite = 0; 
	end
	try options.method
	catch, options.method = 'linear';
	end
	try options.channels
	catch, options.channels = []; 
	end
	try options.newfile
	catch, options.newfile = 0; 
	end
	try options.channel_action
	catch, options.channel_action = 'add'; 
	end
	try options.extrapolate
	catch, options.extrapolate = 0; 
	end
	try options.dont_ask_overwrite
	catch, options.dont_ask_overwrite = 0; 
	end
	try options.trim_nan
	catch, options.trim_nan = 1; 
	end
	...
	elseif ~islogical(options.overwrite) && ~isnumeric(options.overwrite)
		...
		return;
	elseif ~islogical(options.trim_nan) && ~isnumeric(options.trim_nan)
		warning('ID:invalid_input', 'options.trim_nan must be numeric or logical');
		return;
	end
	...
	for d=1:numel(D)
		...
		for k = 1:numel(chans)
			...
			if numel(find(~isnan(dat))) < 2
				...
				% get the remaining NaN back for preserving the length
				dat = [dat; dat_remaining];
