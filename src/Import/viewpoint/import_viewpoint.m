...
% ● Description
%   import_viewpoint is the function for importing raw ViewPoint (.txt) 
%   files to usual PsPM structure.
% ● Format
%   [data] = import_viewpoint(filepath)
% ● Arguments
...
%                channels:  Matrix (timestep x n_cols) of relevant PsPM 
%                           columns. Currently, time, pupil, gaze, blink 
%                           and saccade channels are imported.
...
% ● Copyright
%   Written by Laure Ciernik in 2019
%   Function inspired by GazeAlyze.
%   Most parts rewritten by Eshref Yozdemir to handle newer ViewPoint files.
...
for sn = 1:numel(sess_beg_indices) - 1
  ...
  data{sn}.dataraw_header = transpose(chan_info.read_numeric_columns);
