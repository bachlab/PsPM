...
%
% 1. Pupil preprocessing is performed in two main steps. In the first step,
%    the “valid” samples are determined. The samples that are not valid are
%    not used in the second step. Determining valid samples is done by
%
%    (a) Range filtering: Pupil size values outside a predefined range are
%    considered invalid. This range is configurable.
%
%    (b) Speed filtering: Speed is computed as the 1st difference of pupil
...
%
%    (c) Edge filtering: Samples at both sides of temporal gaps in the data
%    are considered invalid. Both the duration of gaps and the invalid sample
%    duration before/after the gaps are configurable.
%
%    (d) Trendline filtering: A data trend is generated by smoothing and
%    interpolating the data. Then, samples that are too far away from this
%    trend are considered invalid. These two steps are performed multiple
%    times in an iterative fashion. Note that the generated trend is not the
%    final result of this function. The smoothing, invalid threshold and the
%    number of passes are configurable.
%
...
%    are configurable.
%
% 2. In the second step, output smooth signal is generated using the valid samples
%    found in the previous step. This is done by performing filtering, upsampling
...
if isfield(options, 'custom_settings')
  default_settings = assign_fields_recursively(default_settings, ...
...
if combining
  ...
  if ~isempty(chantype_array(ismember(chantype_array,'l')))
    chantype_array(ismember(chantype_array,'l')) = {'b'};
  end
  if ~isempty(chantype_array(ismember(chantype_array,'r')))
    chantype_array(ismember(chantype_array,'r')) = {'b'};
  end
