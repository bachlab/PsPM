...
%         size array normalized by the temporal separation. Samples with 
%         speed higher than a threshold are considered invalid. The 
...
%         interpolating the data. Then, samples that are too far away from 
%         this trend are considered invalid. These two steps are performed 
%         multiple times in an iterative fashion. Note that the generated 
%         trend is not the final result of this function. The smoothing, 
%         invalid threshold and the number of passes are configurable.
%	    (e)	Isolated sample filtering: Isolated and small sample islands are
%		      considered invalid. The size of the islands and the temporal 
%         separation are configurable.
...
% 			      Settings structure to modify the preprocessing steps. If not 
%             specified, the default settings structure obtained from 
%             <a href="matlab:help pspm_pupil_pp_options">pspm_pupil_pp_options</a>
%			        will be used. To modify certain fields of this structure, you
%             only need to specify those fields in custom_settings. For 
%             example, to modify settings.raw.PupilMin, you need to create 
%             a struct with a field .raw.PupilMin.
...
% 	.out_channel
%             Channel ID of the preprocessed output.
% ● References
