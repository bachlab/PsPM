...
% DEFINITION
% pspm_dcm sets up a DCM for skin conductance, prepares and normalises the data, passes it
% over to the model inversion routine, and saves both the forward model and its inversion.
%
% Both flexible-latency (within a response window) and fixed-latency (evoked after a
% specified event) responses can be modelled. For fixed responses, delay and dispersion
% are assumed to be constant (either pre-determined or estimated from the data),
% while for flexible responses, both are estimated for each individual trial.
% Flexible responses can for example be anticipatory, decision-related, or evoked with
% unknown onset.
%
% FORMAT
...
%			model.timing:		  A file name/cell array of events (single session) OR a cell
...
%								        See pspm_get_timing for epoch definition; specify a cell array for
%                       multiple input files.
%								        This must always be specified in SECONDS.
%								        Default: no missing values
%			model.lasttrialcutoff:  If there fewer data after the end of then last trial in a
%                       session than this cutoff value (in s), then estimated
%                       parameters from this trial will be assumed inestimable and set to
%                       NaN after the inversion. This value can be set as inf to always
%                       retain parameters from the last trial.
%                       Default: 7 s
%			model.substhresh:	Minimum duration (in seconds) of NaN periods to cause splitting up
%                       into subsessions which get evaluated independently
...
%								        i.e. Data are normalised during inversion but results transformed
%                       back into raw data units.
%								        Default: 0.
% 		model.constrained:Constrained model for flexible responses which have fixed
...
%			options.rf:			  Call an external file to provide response function
...
%									      in the order they are specified in the model.timing array - to
%                       be used for display and export only
%
% OUTPUT
...
%
% pspm_dcm calculates the inter-trial intervals as the duration between the end of a trial
% and the start of the next one.
% ITI value for the last trial in a session is calculated as the duration between the end
% of the last trial and the end of the whole session.
% Since this value may differ significantly from the regular ITI duration values, it is
% not used when computing the minimum ITI duration of a session.
%
...
for iSn = 1:numel(model.datafile)
  ...
  if isempty(missing{iSn})
    ...
    ignore_epochs = pspm_index2time(diff(miss_epochs, 1, 2),data{iSn}{1}.header.sr) > ...
      model.substhresh;

  else
    % use missing epochs as specified by file
    miss_epochs = pspm_index2time(missing{iSn},data{iSn}{1}.header.sr);
  ...
  if any(ignore_epochs)
    ...
    subsessions(end+(1:n_sbs), 1:4) = [ones(n_sbs,1)*iSn, ...
      pspm_index2time([se_start, se_stop], data{iSn}{1}.header.sr), ...
      zeros(n_sbs,1)];

    % missing epochs
    n_miss = sum(ignore_epochs);
    subsessions(end+(1:n_miss), 1:4) = [ones(n_miss,1)*iSn, ...
      pspm_index2time(miss_epochs(i_e,:),data{iSn}{1}.header.sr), ...
      ones(n_miss,1)];
  else
    subsessions(end+1,1:4) = [iSn, ...
      pspm_index2time([1, numel(data{iSn}{1}.data)], data{iSn}{1}.header.sr), 0];
...
for vs = 1:numel(valid_subsessions)
  ...
  sbs_missing_indx = pspm_time2index(pspm_index2time(find(sbs_miss),data{sbSn(1)}{1}.header.sr), model.sr);
    ...
    for isn_sbs=1:numel(sn_sbs)
      ...
      if sum(sbs_trls)>0 % if any trials exist
        ...
        sbs_iti{sbs_id} = [sbs_trlstart{sbs_id}(2:end); ...
          pspm_index2time(numel(sbs_data{sbs_id, 1}),model.sr)] - sbs_trlstop{sbs_id};
