function [sts, infos, data, filestruct] = pspm_load_data(fn, chan)
	...
	case {'string', 'char'}
		...
		if ~exist(fn, 'file')
			warning('ID:invalid_input', 'The file fn does not exist.');
			return
		end
		...
		loaded_scr = load(fn, 'scr')
		loaded_infos = load(fn, 'infos');
		loaded_data = load(fn, 'data');
		if ~isempty(loaded_scr) % check for SCRalyze 1.x files
			warning('ID:SCRalyze_1_file', 'SCRalyze 1.x compatibility is discontinued');
			return
		end
		if isempty(loaded_infos) || ~isstruct(loaded_infos) || isempty(loaded_data) || ~iscell(loaded_data)
	...
	for k = 1:numel(data)
		...
		if ~isfield(data{k}, 'header')
			...
			if ~isfield(data{k}.header, 'chantype') || ~isfield(data{k}.header, 'sr') || ~isfield(data{k}.header, 'units')
				vflag(k) = 1;
			else ~ismember(lower(data{k}.header.chantype), {settings.chantypes.type})
				nflag(k) = 1;
			end
		end
		% check data
