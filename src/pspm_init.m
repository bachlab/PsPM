...
if str2num(v(1:3))<7.1
  verr=1;
  errmsg=sprintf(['You are running PsPM on a Matlab version (%s) under',...
    'which it has not been tested.\nSPM 8 functions will be ',...
    'automatically added to you path but may not run. 1st level GLM ',...
    'may not run.\nIf you encounter any other error, please contact ',...
    'the developers.'], v);
  warning(errmsg);
end
...
if ~signal
  errmsg=['Signal processing toolbox not installed. ',...
...
if ~isempty(all_paths_spm)
  ...
  if strcmp(questdlg(sprintf(['The software SPM is currently on ',...
      'your MATLAB search path.\n\n' ...
      'Do you want to remove the folders belonging to SPM from your ',...
      'MATLAB search path in order to avoid potential ' ...
      'issues with PsPM?']), ...
    ...
    % quit pspm_init
    errmsg=['Start of PsPM had to be quit, because of interference ',...
      'with the software SPM, which was on your MATLAB search path.',...
      ' To run PsPM be sure to remove the folders of SPM from your ',...
      'MATLAB search path.'];
    error(errmsg);
  end
end
...
if isempty (dummy)
  ...
  if ~isempty(m)
    if strcmp(questdlg(sprintf(['Matlabbatch from SPM and its config ',...
        'folder are currently on your MATLAB search path.\n\n' ...
...
%add channel for saccade right
defaults.chantypes(29) = ...
  struct('type', 'saccade_r', ...
  'description', 'Saccade right', ...
  'import', @pspm_get_saccade_r, ...
  'data', 'wave');

%add channel for (gaze) scanpath speed
...
defaults.chantypes(31) = ...
  struct('type', 'sps_r', ...
  'description', 'Scanpath speed', ...
  'import', @pspm_get_sps_r, ...
  'data', 'wave');

defaults.chantypes(32) = ...
  struct('type', 'sps_l', ...
  'description', 'Scanpath speed', ...
...
% documented
defaults.chantypes(33) = ...
  struct('type', 'custom', ...
  'description', 'Custom', ...
  'import', @pspm_get_custom, ...
  'data', 'wave');
defaults.chantypes(34) = ...
  struct('type', 'pupil_r_pp', ...
  'description', 'Pupil right preprocessed', ...
  'import', @none, ...
  'data', 'wave');
% documented
defaults.chantypes(35) = ...
  struct('type', 'pupil_l_pp', ...
  'description', 'Pupil left preprocessed', ...
  'import', @none, ...
  'data', 'wave');
defaults.chantypes(36) = ...
  struct('type', 'pupil_b_pp', ...
  'description', 'Pupil combined preprocessed', ...
  'import', @none, ...
  'data', 'wave');

% documented
defaults.chantypes(37) = ...
  struct('type', 'gaze_x_b_pp', ...
  'description', 'Gaze x combined preprocessed', ...
  'import', @none, ...
  'data', 'wave');
defaults.chantypes(38) = ...
  struct('type', 'gaze_y_b_pp', ...
  'description', 'Gaze y combined preprocessed', ...
  'import', @none, ...
  'data', 'wave');

defaults.chantypes(39) = ...
  struct('type', 'gaze_x_l_pp', ...
  'description', 'Gaze x left preprocessed', ...
  'import', @none, ...
  'data', 'wave');
defaults.chantypes(40) = ...
  struct('type', 'gaze_y_l_pp', ...
  'description', 'Gaze y left preprocessed', ...
  'import', @none, ...
  'data', 'wave');

defaults.chantypes(41) = ...
  struct('type', 'gaze_x_r_pp', ...
  'description', 'Gaze x right preprocessed', ...
  'import', @none, ...
  'data', 'wave');
defaults.chantypes(42) = ...
  struct('type', 'gaze_y_r_pp', ...
  'description', 'Gaze y right preprocessed', ...
  'import', @none, ...
  'data', 'wave');

defaults.chantypes(43) = ...
  struct('type', 'gaze_x_b', ...
  'description', 'Gaze x combined', ...
  'import', @none, ...
  'data', 'wave');
defaults.chantypes(44) = ...
  struct('type', 'gaze_y_b', ...
  'description', 'Gaze y combined', ...
  'import', @none, ...
  'data', 'wave');

defaults.chantypes(45) = ...
  struct('type', 'sps_b', ...
...
defaults.import.datatypes(9) = ...
  ...
  'chantypes', ...
    {{defaults.importchantypes(~strcmpi('hb',...
    {defaults.importchantypes.type})).type}}, ...  %all except hb
  'chandescription', 'channel', ...
  'multioption', 1, ...
  'searchoption', 1, ...
  'automarker', 1, ...
...
defaults.import.datatypes(11) = ...
  ...
  'chantypes', ...
    {{defaults.importchantypes(~strcmpi('hb',...
    {defaults.importchantypes.type})).type}}, ...
  'chandescription', 'channel', ...
  'multioption', 1, ...
  'searchoption', 0, ...
  'automarker', 1, ...
...
defaults.import.datatypes(15) = ...
  ...
  'chantypes', ...
    {{defaults.chantypes(~strcmpi('hb',...
    {defaults.chantypes.type})).type}}, ...
  'chandescription', 'channel', ...
  'multioption', 1, ...
  'searchoption', 1, ...
  'automarker', 1, ...
...
defaults.import.rsr = 1000;
  % minimum resampling rate for pulse data import
defaults.import.sr = 100;
  % final sampling rate for pulse data import

defaults.import.mat.sr_threshold = 1; 
  % maximum value of the field '.sr' to which data is recognized as timestamps

% Preprocessing settings
% ----------------------
defaults.split.max_sn = 10;
  % split sessions: assume maximum 10 sessions
defaults.split.min_break_ratio = 3;
  % split sessions: assume inter marker intervals 3 times longer for breaks
...
defaults.pfm(1) = struct( ...
  ...
  'cbf', struct('fhandle', @pspm_bf_ldrf_gm, ... % basis function (BF)
  'args', [0.2, 2.40 , 0.29 , 0.77]), ...        % default parameters for BF
  'cif', struct('fhandle', @pspm_bf_ldrf_gm, ... % input function (IF)
  'args', [0, 2.76 , 0.09 , 0.31], ...           % default parameters for IF
