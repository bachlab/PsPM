function [sts, data_cell] = pspm_load_single_chan(fn, chan, which_of_many, desired_type_substr)
    ...
    %
    % Load a single chan from a given datafile.
    %
    % fn: filename
    % chan: chan type
    % which_of_many: What to do if multiple channels are loaded when type is chan. Pass 'first' or 'last'.
    % desired_type_substr: Substring in the desired chan type.
    %
    % sts: Status
    % data_cell: Cell array with single element, containing the loaded PsPM chan.
    %
    % -----------------------------------------------
    ...
    if numel(data_cell) > 1
        warning('ID:multiple_channels', ['There is more than one chan'...
    ...
    if ~contains(channeltype, desired_type_substr)
        warning('ID:invalid_input', sprintf('Loaded channeltype %s does not correspond to a %s chan', channeltype, desired_type_substr));
        return;
    end
    sts = 1;
end
