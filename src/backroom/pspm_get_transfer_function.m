...
% [test resistor value, scr chan number, skipvalues])
%
% where SCR file is either an .smr file or an SCRalyze file
% DEFAULTS:
% test resistors: load from pspm_filtestbox.mat (equivalent to value
% 'filtestbox')
% scr chan number: look for scr chan number
%
% if the calibration dataset contains multiple channels, the SCR chan
% name must contain 'scr', 'gsr' or 'eda', or the chan number must be
