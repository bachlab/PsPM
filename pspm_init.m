...
defaults.import.datatypes(18) = ...
    ...
    'automarker', 1, ...
    'autosr', 1, ...
    'help', ['Eyelink output files (with extension *.edf) must first ', ...
    'be converted to ASCII format (extension *.asc). This is done with ', ...
    'the utility edf2asc.exe (normally included in the Eyelink software ', ...
    'in <Path to Program Files>\SR Research\EyeLink\EDF_Access_API\). ', ...
    'Otherwise there is a Data viewer, available at ', ...
    'http://www.sr-research.com/dv.html (registration needed), ', ...
    'which installs a utility called ''Visual EDF2ASC''. This also allows ', ... 
    'the conversion and does not require a license.', ...
    'The composition of channels depends on the acquisition settings. ', ...
    'If two eyes were recorded channels will be (Channel id 1-8): ', ...
    'Pupil (diameter) L, Pupil R, x L, y L, x R, y R, blink L, blink R. ', ...
    'If one eye was recorded, channels will be (Channel id 1-4): ', ...
    'Pupil (diameter), x, y, blink.']);
...
defaults.glm(3) = ... % GLM for HP (fear-conditioning)
    ...
    'filter', struct('lpfreq', 0.5, 'lporder', 6,  ...
    'hpfreq', 0.015, 'hporder', 6, 'down', 10, ...
    'direction', 'bi'), ...
    'default', 0);
...
defaults.glm(4) = ... % GLM for PS (fear-conditioning)
    ...
    'filter', struct('lpfreq', NaN, 'lporder', NaN, ...
        'hpfreq', NaN, 'hporder', NaN, 'down', 10, ...
        'direction', 'bi'), ...
    'default', 0);
...
defaults.glm(6) = ... % GLM for RA (fear-conditioning)
    ...
    'hpfreq', 0.001, 'hporder', 6, 'down', 10, ...
    'direction', 'bi'), ...
    'default', 0);
...
defaults.glm(7) = ... % GLM for RP (evoked)
    struct('modality', 'rp', ...
    'modelspec', 'rp_e', ...
    'cbf', struct('fhandle', @pspm_bf_rprf_fc, 'args', 1), ...
    'filter', struct('lpfreq', 1, 'lporder', 1,  ...
    'hpfreq', 0.01, 'hporder', 1, 'down', 10, ...
    'direction', 'uni'), ...
    'default', 0);
...
defaults.glm(8) = ... % GLM for RFR (evoked)
    struct('modality', 'rfr', ...
    'modelspec', 'rfr_e', ...
    'cbf', struct('fhandle', @pspm_bf_rfrrf_fc, 'args', 1), ...
    'filter', struct('lpfreq', 1, 'lporder', 1,  ...
...
defaults.dm(1) = ...    
    ...
    'cbf', struct('fhandle', @pspm_bf_sebrf_fc, 'args', 1), ...  % default basis function/set
    'filter', struct('lpfreq', 470, 'lporder', 4,  ...        % default filter settings
    'hpfreq', 50, 'hporder', 4, 'down', 500, ...
    'direction', 'uni'), ...
    'default', 1);
% -------------------------------------------------------------------------
%  DCM settings
