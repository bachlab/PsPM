...
timing.help   = {['Specify the timings of individual events from your design either by creating a timing file ' ...
    ...
    'duration. For each event of this type, specify a time window in which the response is expected. SCRalyze will ' ...
...
condition_rep.check   = @scr_cfg_dcm_check_conditions;
...
dcm.prog = @scr_cfg_run_dcm;
dcm.vout = @scr_cfg_vout_dcm;
dcm.help = {['Non-linear models for SCR are powerful if response timing is not precisely known and has to be ' ...
    ...
    'causal modelling (DCM) is the framework for parameter estimation. SCRalyze implements an iterative ' ...
...
function [sts, val] = scr_cfg_dcm_check_conditions(val)
sts = [];
nrCond = size(val,2);
testVec = [];
for iCond=1:nrCond
