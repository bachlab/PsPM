...
| options.plot | [0(default) or 1] Display a histogram of the delays found and a plot with the detected sound, the trigger and the onset of the sound events. These are color coded for delay, from green (smallest delay) to red (longest). Forces the 'diagnostics' option to true. |
...
| info | See following fields. |
| info.snd_markers | vector of beginning of sound events. |
| info.delays | vector of delays between markers and detected sounds. Only available with option 'diagnostics' turned on. |
