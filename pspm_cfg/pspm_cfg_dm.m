function dm = pspm_cfg_dm
% pspm_cfg_dm
%
% Specifies the Matlabbatch configuration for Dictionary matching models
% for EMG data.
%
% This is forked from pspm_glm (15.11.2016)
...
datafile.help    = {['Add the data file containing the EMG', ...
...
missing.help   = {['Indicate epochs in your data in which the EMG' ...
...
norm.help    = {['Specify if you want to z-normalize the EMG data for each subject. For within-subjects ' ...
...
chan         = cfg_choice;
chan.name    = ['EMG Channel'];
chan.tag     = 'chan';
chan.val     = {chan_def};
chan.values  = {chan_def,chan_nr};
chan.help    = {['Indicate the channel containing the EMG data.'], ['By default ' ...
    'the first EMG channel is assumed to contain the data for this model.'], ...
    ['If the first EMG channel does not contain the data for this', ...
    ' model (e. g. there are two EMG channels), ' ...
    'indicate the the channel number (within the EMG', ...
...
f = strcmpi({settings.dm.modelspec}, 'emg');
def_filt = settings.dm(f).filter;
...
down.num     = [1 1];
down.help    = {['Specify the sampling rate in Hz to down sample EMG data.', ...
...
filter.help   = {['Specify how you want filter the EMG data.']};
...
window.val     = {10};
window.num     = [1 1];
window.help    = {['Indicates (in seconds) over which time window ', ...
    '(starting with the events specified in timing) the model ', ...
    'should be evaluated.']};
...
dm.name  = 'DM for EMG';
dm.tag   = 'dm';
dm.val   = {modelfile, outdir, chan, timeunits, window, session_rep, bf, ...
    norm, filter, overwrite};
%dm_scr.prog  = ;
dm.vout  = @pspm_cfg_vout_dm;
dm.prog = @pspm_cfg_run_dm;
dm.help  = {...
    '', ...
    'References: '
    };

function vout = pspm_cfg_vout_dm(job)
vout = cfg_dep;
