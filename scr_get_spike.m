function [sts, import, sourceinfo] = scr_get_spike(datafile, import)
% scr_get_spike is the main function for import of spike files
% FORMAT: [sts, import, sourceinfo] = scr_get_spike(datafile, import);
% 
...
% $Id: scr_get_spike.m 701 2015-01-22 14:36:13Z tmoser $
% $Rev: 701 $
...
for k = 1:numel(import)
    ...
    if import{k}.channel > 0
        ...
        chan = scr_find_channel(arrayfun(@(i) chanhead{i}.title, 1:numel(chanhead), 'UniformOutput', 0), ...
    ...
    if strcmpi(settings.chantypes(import{k}.typeno).data, 'wave')
        ...
        elseif chanhead{chan}.kind == 3 % timestamps
            ...
            import{k}.data = scr_pulse_convert(chandata{chan}, settings.import.rsr, settings.import.sr);
        ...
        elseif chanhead{chan}.kind == 4 % up and down timestamps
            ...
            pulse = pulse(1:2:end);
            import{k}.data = scr_pulse_convert(pulse, settings.import.rsr, settings.import.sr);
            import{k}.sr = settings.import.sr;
    ...
    elseif strcmpi(settings.chantypes(import{k}.typeno).data, 'events')
        ...
        elseif chanhead{chan}.kind == 4 && strcmpi(import{k}.type, 'marker') % for TTL marker channels with up AND down timestamps
            kbchan = scr_find_channel(arrayfun(@(i) chanhead{i}.title, 1:numel(chanhead), 'UniformOutput', 0), ...
            ...
            if isfield(import{k}, 'denoise') && ~isempty(import{k}.denoise) && import{k}.denoise > 0
                import{k}.data = scr_denoise_spike(chandata{chan}, chanhead{chan}, kbdata, import{k}.denoise);
            else
                pulse = chandata{chan};
                % start with low to high
                if chanhead{chan}.initLow==0
