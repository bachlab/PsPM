function varargout = scr_display(varargin)
% -------------------------------------------------------------------------
...
% Last Modified by GUIDE v2.5 03-Apr-2014 15:10:03

% $Id: scr_display.m 701 2015-01-22 14:36:13Z tmoser $
% $Rev: 701 $
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @scr_display_OpeningFcn, ...
    'gui_OutputFcn',  @scr_display_OutputFcn, ...
    'gui_LayoutFcn',  [], ...
    'gui_Callback',   []);
...
% --- Executes just before scr_display is made visible.
function scr_display_OpeningFcn(hObject, eventdata, handles, varargin)
...
if(numel(varargin)) == 1
    ...
    [foo, foo, handles.data, foo]=scr_load_data(filename,0);
...
% Choose default command line output for scr_display
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes scr_display wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = scr_display_OutputFcn(hObject, eventdata, handles)
...
typ='mat';
sel=[];
mesg='select scr_datafiles to display';
    ...
    if not(isempty(handles.prop.wave))  % only if there is wave info
        ...
        switch handles.prop.wave
            case 'ecg'
                ...
                [sts,wave,foo]=scr_prepdata(wave,filt);
...
% --- Executes when scr_display is resized.
function scr_display_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to scr_display (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
