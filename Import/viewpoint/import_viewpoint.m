function [data] = import_ViewPoint(varargin)
% IMPORT_VIEWPOINT: import of raw ViewPoint (.txt files) to PsPM usual
% structure

% FORMAT:
% data = import_ViewPoint(fn)
%   fn: path to the file which contains the recorded SMI Data File
...
if isempty(varargin)
    warning('ID:invalid_input', 'import_ViewPoint.m needs an imput file with ViewPoint data.'); return;
end
...
if any(Eye_A)&& any(Eye_B)
    ...
    channels_units = {'seconds','normalized','normalized','normalized','normalized','normalized','normalized'};
else
    eyesObserved = 'A';
    col_idx = [2,3,7,4,5];
    channels_header = {'Time','pupil_A','gaze_x_A','gaze_y_A'};
    channels_units = {'seconds','normalized','normalized','normalized'};
end
%% get data columns
