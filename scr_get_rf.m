function theta = scr_get_rf(fn, events, outfile, chan, options)
% scr_get_rf estimates a response function from an event-related design
...
% v002 drb 15.08.2012 instead of copying code, call scr_dcm
% v001 drb 06.08.2012
...
options.chan = chan;
[foo dcm] = scr_dcm(fn, '', events, options);
...
if ~isempty(outfile)
    ...
    job{2}='%-----------------------------------------------------------------------';
    job{3}=['% Response function created by scr_get_rf, ', date, sprintf('  %02.0f:%02.0f', c(4:5))];
    job{4}='%-----------------------------------------------------------------------';
