function varargout = scr_jobman(varargin)
% Main interface for SCR Batch System
%
% FORMAT scr_jobman('initcfg')
% Initialise jobs configuration and set MATLAB path accordingly.
%
% FORMAT scr_jobman('run',job)
% FORMAT output_list = scr_jobman('run',job)
% Run specified job
...
%
% FORMAT job_id = scr_jobman
%        job_id = scr_jobman('interactive')
%        job_id = scr_jobman('interactive',job)
%        job_id = scr_jobman('interactive',job,node)
%        job_id = scr_jobman('interactive','',node)
...
% $Id: scr_jobman.m 701 2015-01-22 14:36:13Z tmoser $
% $Rev: 701 $
...
if isempty(isInitCfg) &&  ~(nargin == 1 && strcmpi(varargin{1},'initcfg'))
    warning('spm:scr_jobman:NotInitialised',...
        'Run scr_jobman(''initcfg''); beforehand');
    scr_jobman('initcfg');
end
isInitCfg = true;
    ...
    case {'interactive'}
        ...
        elseif exist('mod_cfg_id', 'var')
            if isempty(mod_cfg_id)
                arg3 = regexprep(varargin{3},'^spmjobs\.','spm.');
                warning('spm:scr_jobman:NodeNotFound', ...
        ...
        case '.mat'
            try
                ...
                elseif isfield(S,'jobs')
                    ...
                    warning('ID:scr_jobman:JobNotFound','No job found in ''%s''', filenames{cf});
                end
            catch
                warning('ID:scr_jobman:LoadFailed','Load failed: ''%s''',filenames{cf});
            end
        case '.m'
            try
