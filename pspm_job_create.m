function job = pspm_job_create(varargin)
% Create a standard SCRalyze job script from various UI functions
% FORMAT: job = pspm_job_create(fname, varnames, functionargs)
...
% $Id: pspm_job_create.m 701 2015-01-22 14:36:13Z tmoser $
% $Rev: 701 $

% v005 drb 30.07.2012 added cell/cell of char option (for 2-sample t-test
%                     in pspm_con2), added time
...
c = clock;
job{1}='%-----------------------------------------------------------------------';
job{2}=['% Job script created by pspm_job_create, ', date, sprintf('  %02.0f:%02.0f', c(4:5))];
job{3}='%-----------------------------------------------------------------------';
job{4}=' ';
job{5} = 'global settings';
job{6} = 'if isempty(settings), pspm_init; end;';
lines = 7;
