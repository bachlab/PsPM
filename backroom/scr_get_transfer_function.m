function scr_get_transfer_function(infile, outfile, Rs, Rtest, scrchan, skipvalues)
...
%
% FORMAT:
% scr_get_transfer_function(SCR file, output file, serial resistor value, 
% [test resistor value, scr channel number, skipvalues])
%
% where SCR file is either an .smr file or an SCRalyze file
% DEFAULTS: 
% test resistors: load from scr_filtestbox.mat (equivalent to value
% 'filtestbox')
...
switch ext
    case {'.smr'}
        ...
        [data sts] = scr_get_spike(infile, import);
        if sts == -1, warning('Import failed'); return; end;
        scr = data{1}.data;
        sr = data{1}.header.sr;
    case {'.mat'}
        [sts infos data] = scr_load_data(infile, 'scr');
...
c = fmincon('scr_transfer_fit',init_value,[],[],[],[],LB,UB,[],options, R, PR);
...
pos=scr_axpos(2,1,0.05,0.05,0.1,0.2,0.05);
