function [sts, outfile] = scr_scr2ledalab(datafile, outfile, options)
% scr_scr2ledalab is a function for exporting SCRalyze files to ledalab
...
%
% FORMAT: [sts, outfile] = scr_scr2ledalab(datafile, outfile, options)
...
%--------------------------------------------------------------------------
[sts, infos, scr] = scr_load_data(datafile, 'scr');
...
if options.filter == 1
    ...
    filt.down = 10;
   [sts, newscr, newsr] = scr_prepdata(scr{1}.data, filt);
   clear scr
...
[sts, infos, events] = scr_load_data(datafile, 'events');
