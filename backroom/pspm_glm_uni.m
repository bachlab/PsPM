function pspm_glm_uni(datafile, regfile, modelfile, timeunits, basefunctions, normalize, chan, options)

% test script for GLM with unidirectional data filtering DRB 22.8.2011

% pspm_glm models a within subject general linear model of predicted
...
% v107 drb 8.12.2009 deleted disable - now moved to pspm_prepdata
...
for d=1:numel(datafile)
    ...
    [pspm_down sr] = pspm_prepdata(scr, 'uni');
...
if ischar(basefunctions),
    switch lower(basefunctions)
        case 'fir', bf.f=str2func('pspm_bf_FIR'); bf.arg = [1/glm.sr.fin, 30];
        case 'scrf', bf.f=str2func('pspm_bf_infbs');  bf.arg = [1/glm.sr.fin, 0];
        case 'scrf1', bf.f=str2func('pspm_bf_infbs'); bf.arg = [1/glm.sr.fin, 1];
        case 'scrf2', bf.f=str2func('pspm_bf_infbs'); bf.arg = [1/glm.sr.fin, 2];
...
for k = 1:(size(glm.X, 2) - d)
    glm.X(:, k) = pspm_prepdata(glm.X(:, k), glm.sr.fin, settings.lp, settings.hp, 'none', 'uni');
end;
