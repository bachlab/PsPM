function scr_peakscore(datafile, regfile, modelfile, timeunits, normalize, chan, options)

% scr_peakscore calculates event-related responses by scoring the peak
% response against a pre-stimulus baseline. The input is similar to
% scr_glm, and the output is written into a DUMMY glm structure to make it
% readable for scr_con1
%
% FORMAT:
...
for d=1:numel(datafile)
    ...
    [sts, scr_down, sr] = scr_prepdata(scr.data, model.filter);
