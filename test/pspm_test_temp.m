function pspm_test_temp(varargin)
	%% pspm_test_temp is a wrapper script for testing all testable functions in one
	...
	[pth, ~, ~] = fileparts(which('pspm_test_temp.m'));
  if strcmp(pth(end-3:end),'test'); pth = [pth(1:end-4),'src']; end
	addpath(pth);
	pspm_init;
