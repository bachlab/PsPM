...
% (C) 2013 Linus Rüttimann (University of Zurich)
    ...
    methods (Test)
        function invalid_inputargs(this)
            datafile = 'string';
            datatype = 'spike';
            this.verifyWarning(@()pspm_import(datafile, datatype), 'ID:invalid_input', 'invalid_input test 1'); %no import variable
            import = 'foo';
            this.verifyWarning(@()pspm_import(datafile, datatype, import), 'ID:invalid_input', 'invalid_input test 2'); %no cell or struct import variable
            clear import;
            import{1}.type = 'scr';
            datatype = 'foo';
            this.verifyWarning(@()pspm_import(datafile, datatype, import), 'ID:invalid_channeltype', 'invalid_input test 3'); %invalid channeltype
            datatype = 'spike';
            datafile = 5;
            this.verifyWarning(@()pspm_import(datafile, datatype, import), 'ID:invalid_input', 'invalid_input test 4'); %no char filname
        ...
        function invalid_import_struct(this)
            ...
            import{1}.type = 'scr';
            import{2}.type = 'hb';
            this.verifyWarning(@()pspm_import(datafile, datatype, import), 'ID:ivalid_import_struct', 'invalid_import_struct test 1'); %datatype doesn't support multiple channels
            
            clear import;
            import{1}.type = 'hr';
            this.verifyWarning(@()pspm_import(datafile, datatype, import), 'ID:ivalid_import_struct', 'invalid_import_struct test 2'); %not allowed channel type
            
            datatype = 'mat';
            this.verifyWarning(@()pspm_import(datafile, datatype, import), 'ID:ivalid_import_struct', 'invalid_import_struct test 3'); %no samplerate given
            
            datafile = 'foo';
            import{1}.sr = 100;
            this.verifyWarning(@()pspm_import(datafile, datatype, import), 'ID:nonexistent_file', 'invalid_import_struct test 4');
