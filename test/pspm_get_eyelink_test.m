classdef pspm_get_eyelink_test < pspm_get_superclass
  ...
  methods
    ...
    function define_testcases(this)
      ...
      this.testcases{1}.import{1} = struct('type', 'pupil_l', 'channel', 1); 
	  % pupil L
      this.testcases{1}.import{2} = struct('type', 'pupil_r', 'channel', 2); 
	  % pupil R
      % if 0 is specified as channel -> use default value
      this.testcases{1}.import{3} = struct('type', 'marker', 'channel', 0); 
	  % marker
      this.testcases{1}.import{4} = struct('type', 'gaze_x_l', 'channel', 3); 
	  % x l
      this.testcases{1}.import{5} = struct('type', 'gaze_y_l', 'channel', 4); 
	  % y l
      this.testcases{1}.import{6} = struct('type', 'gaze_x_r', 'channel', 5); 
	  % x r
      this.testcases{1}.import{7} = struct('type', 'gaze_y_r', 'channel', 6); 
	  % y r
      this.testcases{1}.import{8} = struct('type', 'blink_l', 'channel', 7); 
	  % blink l
      this.testcases{1}.import{9} = struct('type', 'blink_r', 'channel', 8); 
	  % blink r
      this.testcases{1}.import{10} = struct('type', 'saccade_l', 'channel', 9);
	  % saccade l
      this.testcases{1}.import{11} = struct('type', 'saccade_r', 'channel', 10);
	  % saccade r
    end
    ...
    function [import_struct,channel_types] = set_import_values(this)
      ...
      import_struct{1} =  struct('type', 'pupil_l', 'channel', 1);
	  % pupil L
      import_struct{2} =  struct('type', 'pupil_r', 'channel', 2);
	  % pupil R
      import_struct{3} =  struct('type', 'gaze_x_l', 'channel', 3);
	  % x l
      import_struct{4} =  struct('type', 'gaze_y_l', 'channel', 4);
	  % y l
      import_struct{5} =  struct('type', 'gaze_x_r', 'channel', 5);
	  % x r
      import_struct{6} =  struct('type', 'gaze_y_r', 'channel', 6);
	  % y r
      import_struct{7} =  struct('type', 'blink_l', 'channel', 7);
	  % blink l
      import_struct{8} =  struct('type', 'blink_r', 'channel', 8);
	  % blink r
      import_struct{9} =  struct('type', 'saccade_l', 'channel', 9);
	  % saccade l
      import_struct{10} = struct('type', 'saccade_r', 'channel', 10);
	  % saccade r
      import_struct{11} = struct('type', 'marker', 'channel', 0);
	  % marker
        ...
        switch channel_types{i}
          case 'pupil'
            this.verifyMatches(lower(data{i}.units),...
             '^arbitrary (area|diameter) units$',...
              'The pupil unit is not within the valid values.');
          case 'pupil_mm'
            this.verifyMatches(lower(data{i}.units),...
             '^mm$',...
             'The pupil unit is not mm.');
          case 'position'
            this.verifyMatches(lower(data{i}.units),...
             '^(pixel)$',...
             'The position unit is not within the valid values.');
          case 'blink'
            this.verifyMatches(lower(data{i}.units),...
             '^(blink)$',...
             'The blink unit is not within the valid values.');
          case 'saccade'
            this.verifyMatches(lower(data{i}.units),...
             '^(saccade)$',...
             'The saccade unit is not within the valid values.');
        end
  ...
  methods (Test)
    ...
    function test_multi_session(this)
      ...
      [sts, data, sourceinfo] = pspm_get_eyelink(pth, import);
