classdef pspm_get_eyelink_test < pspm_get_superclass
  ...
  methods
    function define_testcases(this)
      ...
      this.testcases{1}.import{1} = struct('type', 'pupil_l', 'chan', 1); % pupil L
      this.testcases{1}.import{2} = struct('type', 'pupil_r', 'chan', 2); % pupil R
      % if 0 is specified as channel -> use default value
      this.testcases{1}.import{3} = struct('type', 'marker', 'chan', 0); % marker
      this.testcases{1}.import{4} = struct('type', 'gaze_x_l', 'chan', 3); % x l
      this.testcases{1}.import{5} = struct('type', 'gaze_y_l', 'chan', 4); % y l
      this.testcases{1}.import{6} = struct('type', 'gaze_x_r', 'chan', 5); % x r
      this.testcases{1}.import{7} = struct('type', 'gaze_y_r', 'chan', 6); % y r
      this.testcases{1}.import{8} = struct('type', 'blink_l', 'chan', 7); % blink l
      this.testcases{1}.import{9} = struct('type', 'blink_r', 'chan', 8); % blink r
      this.testcases{1}.import{10} = struct('type', 'saccade_l', 'chan', 9); % saccade l
      this.testcases{1}.import{11} = struct('type', 'saccade_r', 'chan', 10); % saccade r
    end
    % function determines import values and channel types
    function [import_struct,channel_types] = set_import_values(this)
      % if 0 is specified as channel -> use default value
      import_struct{1} =  struct('type', 'pupil_l', 'chan', 1); % pupil L
      import_struct{2} =  struct('type', 'pupil_r', 'chan', 2); % pupil R
      import_struct{3} =  struct('type', 'gaze_x_l', 'chan', 3); % x l
      import_struct{4} =  struct('type', 'gaze_y_l', 'chan', 4); % y l
      import_struct{5} =  struct('type', 'gaze_x_r', 'chan', 5); % x r
      import_struct{6} =  struct('type', 'gaze_y_r', 'chan', 6); % y r
      import_struct{7} =  struct('type', 'blink_l', 'chan', 7); % blink l
      import_struct{8} =  struct('type', 'blink_r', 'chan', 8); % blink r
      import_struct{9} =  struct('type', 'saccade_l', 'chan', 9); % saccade l
      import_struct{10} = struct('type', 'saccade_r', 'chan', 10); % saccade r
      import_struct{11} = struct('type', 'marker', 'chan', 0); % marker
      % channel types
  ...
  methods (Test)
    ...
    function test_one_eye(this)
      ...
      % if 0 is specified as channel -> use default value
      import{1} = struct('type', 'marker', 'chan', 0); % marker
      import{1} = struct('type', 'pupil_r', 'chan', 1); % pupil
      import{2} = struct('type', 'gaze_x_r', 'chan', 2); % x
      import{3} = struct('type', 'gaze_y_r', 'chan', 3); % y
      import{4} = struct('type', 'blink_r', 'chan', 4); % blink
      import{5} = struct('type', 'saccade_r', 'chan', 5); % saccade
      % channel types
      channel_types{1} = 'pupil';
    ...
    function test_track_dist(this)
      ...
      import{1} = struct('type', 'pupil_l', ...
        'chan', 1, 'eyelink_trackdist', 700, ...
        'distance_unit', 'mm'); % pupil L
      import{2} = struct('type', 'pupil_r', ...,
        'chan', 2, 'eyelink_trackdist', 700, ...
