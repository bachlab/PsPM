...
% (C) 2014 Tobias Moser (University of Zurich)
    ...
    methods
        ...
        function verify_basic_data_structure(this, data, sourceinfo, channel_types)
            this.verifyMatches(sourceinfo.time, '^\d{2}:\d{2}:\d{2}$', 'The specified sourceinfo time format does not fit into the expected pattern.');
            this.verifyMatches(sourceinfo.date, '^\d{2}.\d{2}.\d{4}$', 'The specified sourceinfo date format does not fit into the expected pattern.');
            ...
            for i = 1:length(channel_types)
                ...
                switch channel_types{i} 
                    case 'pupil' 
                        this.verifyMatches(lower(data{i}.units), '^arbitrary (area|diameter) units$', 'The pupil unit is not within the valid values.');
                    case 'pupil_mm' 
                        this.verifyMatches(lower(data{i}.units), '^mm$', 'The pupil unit is not mm.');
                    case 'position'
                        this.verifyMatches(lower(data{i}.units), '^(pixel)$', 'The position unit is not within the valid values.');
                    case 'blink'
                        this.verifyMatches(lower(data{i}.units), '^(blink)$', 'The blink unit is not within the valid values.');
                    case 'saccade'
                        this.verifyMatches(lower(data{i}.units), '^(saccade)$', 'The saccade unit is not within the valid values.');
                end
    ...
    methods (Test)
        ...
        function invalid_datafile(this)
            ...
            this.verifyWarning(@()pspm_get_eyelink(fn, import), 'ID:channel_not_contained_in_file');
        ...
        function test_blinks_saccades_are_NaN(this)
            ...
            indices = {{{3, 4}, 1}, {{5, 6}, 2}};
            for fn = this.files
                [sts, data, sourceinfo] = pspm_get_eyelink(fn{1}, import);
                assert(sts == 1);
