        ...
        function valid_datafile(this)
            ...
            for k = 1:numel(this.testcases)
                ...
                if isprop(this, 'blocks') && this.blocks
                    ...
                    % import
                    this.verifyEqual(numel(import), numel(sourceinfo), sprintf('Number of blocks differs between sourceinfo and import in testacase %i',k));
                else
                    blkno = 1;
                    import = {import};
                ...
                for blk = 1:blkno
                    ...
                    this.verifyNumElements(import{blk}, numel(this.testcases{k}.import), sprintf('The number of elements of ''import'' does not match in testcase %i', k));

                    for m = 1:numel(this.testcases{k}.import)
                        
                        % test if data exists and has correct datatype
                        this.verifyTrue(isfield(import{blk}{m}, 'data'), sprintf('There is no field ''data'' in importjob %i in testcase %i', m, k));
                        this.verifyTrue(isnumeric(import{blk}{m}.data) || islogical(import{blk}{m}.data), sprintf('The field ''data'' in importjob %i in testcase %i is not numeric', m, k));
                        this.verifyTrue(isvector(import{blk}{m}.data), sprintf('The field ''data'' in importjob %i in testcase %i is not a vector', m, k));
                        
                        % check if sr exists and has a correct datatype                            
                        this.verifyTrue(isfield(import{blk}{m}, 'sr'), sprintf('There is no field ''sr'' in importjob %i in testcase %i', m, k));
                        this.verifyTrue(isnumeric(import{blk}{m}.sr), sprintf('The field ''sr'' in importjob %i in testcase %i is not numeric', m, k));
                        this.verifyTrue(numel(import{blk}{m}.sr) == 1, sprintf('The field ''sr'' in importjob %i in testcase %i is not a number', m, k));
                        ...
                        if strcmpi(settings.chantypes(this.testcases{k}.import{m}.typeno).data, 'wave') || strcmpi(import{blk}{m}.marker, 'continuous')
                            this.verifyTrue(1 <= import{blk}{m}.sr && import{blk}{m}.sr <= 10000, sprintf('The samplerate of wave importjob %i in testcase %i is out of the range [1,10000]', m, k));
                            this.verifyTrue(numel(import{blk}{m}.data) / import{blk}{m}.sr <= 3600, sprintf('The duration of the data of wave importjob %i in testcase %i is longer then 3600s', m, k))
                        elseif strcmpi(settings.chantypes(this.testcases{k}.import{m}.typeno).data, 'events') && (strcmpi(import{blk}{m}.marker, 'timestamps') || strcmpi(import{blk}{m}.marker, 'timestamp'))
                            this.verifyTrue(10^-6 <= import{blk}{m}.sr && import{blk}{m}.sr <= 1, sprintf('The samplerate of event importjob %i in testcase %i is out of the range [10^-6,1]', m, k));
                            this.verifyTrue(max(import{blk}{m}.data) * import{blk}{m}.sr <= 3600, sprintf('The duration of the data of wave importjob %i in testcase %i is longer then 3600s', m, k))
                        else
                            warning('Invalid channel type! (settings.chantypes.data{%i} is neither ''wave'' nor ''events'')', chantype);
                        end
                        
                        if strcmpi(settings.chantypes(this.testcases{k}.import{m}.typeno).data, 'events') && (strcmpi(import{blk}{m}.marker, 'timestamps') || strcmpi(import{blk}{m}.marker, 'timestamp'))
                    ...
                    if strcmpi(settings.chantypes(this.testcases{k}.import{1}.typeno).data, 'events') && (strcmpi(import{blk}{1}.marker, 'timestamps') || strcmpi(import{blk}{1}.marker, 'timestamp'))
