...
% (C) 2013 Linus Rüttimann (University of Zurich)
    ...
    methods
        function define_testcases(this)
            ...
            %--------------------------------------------------------------
            this.testcases{1}.pth = 'testdatafile79887.mat';
            ...
            %--------------------------------------------------------------
            this.testcases{2}.pth = 'testdatafile65786.mat';
    ...
    methods (Test)
        function invalid_datafile(this)
            fn = 'testdatafile13548.mat';
            
            a = 3;
            save(fn, 'a');
            this.verifyWarning(@()pspm_get_mat(fn, this.testcases{1}.import), 'ID:invalid_data_structure', 'invalid_datafile test 1');
            
            data = 'string';
            save(fn, 'data');
            this.verifyWarning(@()pspm_get_mat(fn, this.testcases{1}.import), 'ID:invalid_data_structure', 'invalid_datafile test 2');
            
            data = cell(2,1);
            data{1} = ones(50,1);
            data{2} = ones(50,2);
            save(fn, 'data');
            this.verifyWarning(@()pspm_get_mat(fn, this.testcases{1}.import), 'ID:invalid_data_structure', 'invalid_datafile test 3');
