...
function ChannelActionAddAndReplaceTest(this)
    % Tests if channel_action add and replace behave correctly

    channel1 = 1;
    Exp = this.expansion;
    fn = this.data_filename;

    % Restore clean datafile before test
    copyfile(this.backup_data_filename, fn);
    ...
    [sts, channel_index] = pspm_expand_epochs(fn, channel1, Exp, options);
    this.verifyEqual(sts, 1)

    [~, ~, ~, filestruct2] = pspm_load_data(fn, 'none');
    numofchanafteradd = filestruct2.numofchan;

    this.verifyEqual(numofchanbefor + 1, numofchanafteradd)
    this.verifyEqual(channel_index, numofchanafteradd)
    ...
    [~, ~, ~, filestruct3] = pspm_load_data(fn, 'none');
    numofchanafterreplace = filestruct3.numofchan;

    this.verifyEqual(numofchanafteradd, numofchanafterreplace)
    this.verifyEqual(channel_index, channel1)
